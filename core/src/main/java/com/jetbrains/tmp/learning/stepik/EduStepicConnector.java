...
public class EduStepicConnector {
    ...
    static <T> T getFromStepik(String link, final Class<T> container) throws IOException {
        ...
        Gson gson = new GsonBuilder().registerTypeAdapter(TaskFile.class,
    ...
    public static boolean enrollToCourse(final int courseId) {
        ...
        try {
            final StepikWrappers.EnrollmentWrapper enrollment = new StepikWrappers.EnrollmentWrapper(String.valueOf(
                    courseId));
    ...
    private static boolean addCoursesFromStepik(List<CourseInfo> result, int pageNumber) throws IOException {
        final String url = pageNumber == 0 ?
                ...
                EduStepikNames.COURSES_FROM_PAGE + String.valueOf(pageNumber);
        final StepikWrappers.CoursesContainer coursesContainer = getFromStepik(url,
        ...
        for (CourseInfo info : courseInfos) {
            ...
            if (info.isAdaptive() || (typeLanguage.size() == 2 && PYCHARM_PREFIX.equals(typeLanguage.get(0)))) {
                for (Integer instructor : info.instructors) {
                    final StepikUser author = getFromStepik(EduStepikNames.USERS + "/" + String.valueOf(instructor),
    ...
    public static List<Lesson> getLessons(int sectionId) throws IOException {
        final StepikWrappers.SectionContainer
                sectionContainer = getFromStepik(EduStepikNames.SECTIONS + String.valueOf(sectionId),
        ...
        for (Integer unitId : unitIds) {
            StepikWrappers.UnitContainer
                    unit = getFromStepik(EduStepikNames.UNITS + "/" + String.valueOf(unitId),
                    StepikWrappers.UnitContainer.class);
            int lessonID = unit.units.get(0).lesson;
            StepikWrappers.LessonContainer
                    lesson = getFromStepik(EduStepikNames.LESSONS + String.valueOf(lessonID),
