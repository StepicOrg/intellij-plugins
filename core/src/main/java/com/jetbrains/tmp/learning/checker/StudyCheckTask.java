...
public class StudyCheckTask extends com.intellij.openapi.progress.Task.Backgroundable {
    ...
    public StudyCheckTask(
            Project project,
            StudyState studyState,
            Ref<Boolean> checkInProcess,
            Process testProcess,
            String commandLine) {
    ...
    private StudyTestsOutputParser.TestsOutput getTestOutput(@NotNull ProgressIndicator indicator) {
        ...
        if (indicator.isCanceled()) {
            ApplicationManager.getApplication().invokeLater(
                    () -> StudyCheckUtils.showTestResultPopUp("Check cancelled",
            ...
            if (testOutput.isSuccess()) {
                ...
                if (pair != null && !(!pair.getFirst() && pair.getSecond().isEmpty())) {
                    ...
                    ApplicationManager.getApplication()
                            .invokeLater(() -> StudyCheckUtils.showTestResultPopUp(FAILED_CHECK_LAUNCH,
                                    MessageType.WARNING
                                            .getPopupBackground(),
                                    myProject));
                }
    ...
    protected void onTaskFailed(String message) {
        ...
        if (course != null) {
            if (course.isAdaptive()) {
                ...
                ApplicationManager.getApplication()
                        .invokeLater(() -> StudyCheckUtils.showTestResultPopUp(message,
                                MessageType.ERROR.getPopupBackground(),
                                myProject));
            }
        }
