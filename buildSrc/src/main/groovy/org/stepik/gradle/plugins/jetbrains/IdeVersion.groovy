...
class IdeVersion {
    ...
    static IdeVersion fromString(@NotNull String version) {
        ...
        if (BUILD_NUMBER == version || SNAPSHOT == version) {
            IdeVersion fallback = fromString(FALLBACK_VERSION)
            if (fallback == null) {
                return null
            }
        ...
        def code = version
        def productSeparator = code.lastIndexOf('-') //some products have multiple parts, e.g. "FB-IC-143.157"

        if (productSeparator > 0) {
            code = code.substring(productSeparator + 1)
        }

        def baselineVersionSeparator = code.indexOf('.')
        def baselineVersion
        def buildNumber
        ...
        if (baselineVersionSeparator > 0) {
            ...
            def intComponents = new int[intComponentsList.size()]
