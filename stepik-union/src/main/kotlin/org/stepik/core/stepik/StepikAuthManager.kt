...
object StepikAuthManager {
    ...
    const val implicitGrantUrl = "https://stepik.org/oauth2/authorize/" +
            "?client_id=" + CLIENT_ID +
            "&response_type=token"
    ...
    private fun initStepikApiClient(): StepikApiClient {
        ...
        val jre = System.getProperty("java.version")
        val userAgent = String.format("Stepik Union/%s (%s) StepikApiClient/%s %s/%s JRE/%s",
                PluginUtils.version,
                osName,
                StepikApiClient.getVersion(),
                PluginUtils.getCurrentProduct(),
                PluginUtils.getCurrentProductVersion(),
                jre)
    ...
    private fun getTokenInfo(userId: Long, client: StepikApiClient = stepikApiClient): TokenInfo {
        ...
        val serviceName = StepikProjectManager::class.java.name
        val attributes = CredentialAttributes(serviceName,
                userId.toString(),
                StepikProjectManager::class.java,
                false)
    ...
    private fun setTokenInfo(userId: Long, tokenInfo: TokenInfo) {
        ...
        attributes = CredentialAttributes(serviceName, userName, StepikProjectManager::class.java, false)
