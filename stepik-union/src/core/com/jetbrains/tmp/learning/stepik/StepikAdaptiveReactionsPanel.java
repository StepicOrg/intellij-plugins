...
public class StepikAdaptiveReactionsPanel extends JPanel {
  ...
  private void addFileListener() {
    final FileEditorManagerListener editorManagerListener = new FileEditorManagerListener() {
      ...
      public void selectionChanged(@NotNull FileEditorManagerEvent event) {
        final core.com.jetbrains.tmp.learning.courseFormat.Task task = StudyUtils.getTaskFromSelectedEditor(myProject);
  ...
  private class ReactionButtonPanel extends JPanel {
    ...
    public ReactionButtonPanel(@NotNull final String text,
                               ...
                               int reaction) {
      core.com.jetbrains.tmp.learning.courseFormat.Task task = StudyUtils.getTaskFromSelectedEditor(myProject);
      final boolean isEnabled = task != null && task.getStatus() != StudyStatus.Solved;
    ...
    private class ReactionMouseAdapter extends MouseAdapter {
      ...
      public void mouseClicked(MouseEvent e) {
        if (e.getClickCount() == 1) {
          final core.com.jetbrains.tmp.learning.courseFormat.Task task = StudyUtils.getCurrentTask(myProject);
