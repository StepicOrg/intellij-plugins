...
class DirectivesUtilsKtTest {
    companion object {
        private const val TESTS_COUNT = 7
        private const val SOURCE = 0
        private const val EXPECTED = 1
        private const val EXPECTED_REPLACED = 2

        @DataPoints("languages")
        @JvmField
        val languages = listOf(SupportedLanguages.JAVA8, SupportedLanguages.PYTHON3)
    }

    private val sourcesMap: MutableMap<SupportedLanguages, List<List<String>>> = mutableMapOf()

    private fun readTestFile(fileName: String): String {
        val inputStream = javaClass.getResourceAsStream("/samples/$fileName")
    ...
    fun containsDirectivesJava(@FromDataPoints("languages") language: SupportedLanguages) {
        ...
        for (index in 1 until TESTS_COUNT) {
